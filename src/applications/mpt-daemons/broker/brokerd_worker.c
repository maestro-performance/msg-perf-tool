...
static void *brokerd_handle_set(const maestro_note_t *request, maestro_note_t *response, 
...
static void *brokerd_handle_flush(const maestro_note_t *request, maestro_note_t *response, 
	const maestro_player_info_t *pinfo) 
{
	logger_t logger = gru_logger_get();
...
static void *brokerd_handle_ping(const maestro_note_t *request, maestro_note_t *response, 
	const maestro_player_info_t *pinfo) 
{
	logger_t logger = gru_logger_get();
...
static void *brokerd_handle_start(const maestro_note_t *request, maestro_note_t *response, 
	const maestro_player_info_t *pinfo) 
{
	logger_t logger = gru_logger_get();
...
static void *brokerd_handle_stop(const maestro_note_t *request, maestro_note_t *response, 
	const maestro_player_info_t *pinfo) 
{
	logger_t logger = gru_logger_get();
...
static void *brokerd_handle_stats(const maestro_note_t *request, maestro_note_t *response,
	const maestro_player_info_t *pinfo)
{
	maestro_note_set_cmd(response, MAESTRO_NOTE_OK);
	return NULL;
...
static bool brokerd_abort_check(bmic_queue_stat_t *qstats) {
	if (worker_options.duration_type == MESSAGE_COUNT) {
		if ((qstats->msg_ack_count + qstats->msg_exp_count) >= worker_options.duration.count) {
			return true;
		} 
	}
	else {
...
static bool brokerd_collect(gru_status_t *status) {
	...
	snprintf(filename, sizeof(filename) - 1, "broker-jvm-inspector-%d.csv.gz", getpid());
