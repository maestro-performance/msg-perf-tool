 ...
 */
static void naming_csv_name(naming_info_t *nm_info, const char *basename, char *out, size_t len) 
{
	snprintf(out, len - 1, "%s-%s-%d-%d.csv.gz", nm_info->source, basename, 
		nm_info->pid, nm_info->ppid);
}

static bool naming_initialize_csv_latency_writer(stats_writer_t *writer, 
	naming_info_t *nm_info, gru_status_t *status)
{
...
static bool naming_initialize_csv_writer(stats_writer_t *writer, naming_opt_t nm_opt, 
	void *payload, gru_status_t *status) 
{
	if (nm_opt & NM_LATENCY) {
		if (!naming_initialize_csv_latency_writer(writer, (naming_info_t *) payload, status)) {
			return false;
		}
	}

	if (nm_opt & NM_THROUGHPUT) {
		if (!naming_initialize_csv_throughput_writer(writer, (naming_info_t *) payload, status)) {
			return false;
		}
	}
...
bool naming_initialize_writer(stats_writer_t *writer, report_format_t format, 
	naming_opt_t nm_opt, void *payload, gru_status_t *status) 
{
