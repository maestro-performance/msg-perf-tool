...
static FILE *open_stats_file(
	const char *prefix, const char *suffix, gru_status_t *status) {
...
stat_io_t *statistics_init(stat_direction_t direction, gru_status_t *status) {
	...
	if (!ret->throughput) {
		logger(
			ERROR, "Unable to initialize the statistics IO engine: %s", status->message);
...
void statistics_latency_data(
	stat_io_t *stat_io, uint64_t latency, const char *time, int32_t milli) {
