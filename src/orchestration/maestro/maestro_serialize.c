...
static bool maestro_deserialize_note_assign(const msgpack_object obj, void *out, gru_status_t *status) {
	switch (obj.type) {
		...
		case MSGPACK_OBJECT_STR: {
			snprintf((char *) out, obj.via.str.size + 1, "%.*s", obj.via.str.size,
				obj.via.str.ptr);
			break;
		}
...
static bool maestro_deserialize_note_set_request(const msg_content_data_t *in,
	maestro_note_t *note, msgpack_unpacked *msg, size_t *offset, gru_status_t *status)
{
	...
	if (ret != MSGPACK_UNPACK_SUCCESS) {
		gru_status_set(status, GRU_FAILURE, "Unable to unpack set command: invalid and/or missing command");

		return false;
	}

	if (!maestro_deserialize_note_assign(msg->data, &note->payload->request.set.opt, status))
	{
		return false;
	}
	...
	if (ret != MSGPACK_UNPACK_SUCCESS) {
		gru_status_set(status, GRU_FAILURE, "Unable to unpack set value: invalid and/or missing value");
	...
	if (!maestro_deserialize_note_assign(msg->data, note->payload->request.set.value,
		status))
	{
		return false;
	}
